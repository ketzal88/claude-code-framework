...
Non-blocking by design. When the working tree has .ts/.tsx/.js/.mjs/.cjs changes,
it spawns `graphify update .` in a DETACHED background process and returns
immediately, so the turn does not wait on the ~60s AST re-extraction. A lockfile
(graphify-out/.refresh.lock) prevents overlapping refreshes.

Always exits 0 (a stale graph must never block a turn). Bypass: SKIP_GRAPHIFY_REFRESH=1.
ASCII-only messages (windows-environment.md).
"""
import os
import subprocess
import sys
...
LOCK_TTL = 600  # seconds; a refresh older than this is considered stale
...
def main():
    ...
    # No graph yet -> nothing to keep fresh.
    ...
    # Detached child: run the refresh, then clear the lock. Independent of the
    # session shell so it survives the turn ending.
    inner = (
        "import subprocess, os;"
        "subprocess.run(['python','-m','graphify','update','.'], cwd=r'{root}');"
